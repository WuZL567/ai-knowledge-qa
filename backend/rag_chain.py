"""
AI 知识库问答 Demo —— 把检索接进对话（手写版 RAG Chain）

================================================================================
先读这个：rag.py 负责「检索」那半条链路，这个文件补另一半
================================================================================
...
实现下面 4 个函数（3 个必需 + 自测），然后 `python rag_chain.py` 跑自测。
    ...
    ④ to_sources_frame()       —— 把检索结果编成 SSE 帧
...
if __name__ == "__main__":
    ...
    #      看能不能解析回去、中文有没有乱码
    #
    # 顺序建议：先单独测每个函数，最后再串起来 —— 这是踩坑换来的教训，别忘。
